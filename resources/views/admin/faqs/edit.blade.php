    ...
    <section class="py-10 px-4">
        <div class="max-w-3xl mx-auto bg-white rounded-[36px] border border-pink-100 p-8 shadow-sm">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit FAQ Produk</h1>
                    <p class="text-gray-600 mt-2">Perbarui pertanyaan umum untuk halaman detail produk.</p>
                </div>
            ...
            <form action="{{ route('admin.faqs.update', $faq->id) }}" method="post" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="produk_id" class="w-full rounded-2xl border border-pink-200 bg-pink-50 px-4 py-3">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ old('produk_id', $faq->produk_id) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="pertanyaan" value="{{ old('pertanyaan', $faq->pertanyaan) }}" class="w-full rounded-2xl border border-pink-200 bg-pink-50 px-4 py-3" placeholder="Contoh: Apakah produk ini cocok untuk kulit sensitif?">
                ...
                <div>
                    ...
                    <textarea name="jawaban" rows="5" class="w-full rounded-2xl border border-pink-200 bg-pink-50 px-4 py-3" placeholder="Masukkan jawaban untuk pertanyaan ini">{{ old('jawaban', $faq->jawaban) }}</textarea>
