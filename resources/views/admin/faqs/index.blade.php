        ...
        <div class="max-w-6xl mx-auto">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">Kelola pertanyaan umum produk yang muncul di halaman detail produk.</p>
                </div>
                <a href="{{ route('admin.faqs.create') }}" class="inline-flex items-center gap-2 rounded-full bg-pink-600 px-5 py-3 text-sm font-semibold text-white shadow-lg hover:bg-pink-700 transition">
                    ...
                    <tbody class="divide-y divide-pink-100">
                        @forelse($faqs as $faq)
                            <tr class="hover:bg-pink-50">
                                <td class="px-6 py-4 text-sm text-gray-800">{{ $faq->product->name ?? 'Produk dihapus' }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700 space-x-2">
                                    <a href="{{ route('admin.faqs.edit', $faq->id) }}" class="inline-flex items-center gap-2 rounded-full bg-pink-100 px-4 py-2 text-pink-700 hover:bg-pink-200 transition">Edit</a>
                                    <form action="{{ route('admin.faqs.destroy', $faq->id) }}" method="post" class="inline-block" onsubmit="return confirm('Hapus FAQ ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-8 text-center text-gray-500">Belum ada FAQ produk.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
