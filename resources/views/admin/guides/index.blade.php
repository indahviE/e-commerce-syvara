    ...
    <section class="py-10 px-4">
        <div class="max-w-6xl mx-auto">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <a href="{{ route('admin.guides.create') }}" class="inline-flex items-center gap-2 rounded-full bg-pink-600 px-5 py-3 text-sm font-semibold text-white shadow-lg hover:bg-pink-700 transition">
                    ...
                    <tbody class="divide-y divide-pink-100">
                        @forelse($guides as $guide)
                            <tr class="hover:bg-pink-50">
                                <td class="px-6 py-4 text-sm text-gray-800">{{ $guide->product->name ?? 'Produk dihapus' }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700 space-x-2">
                                    ...
                                    <form action="{{ route('admin.guides.destroy', $guide->id) }}" method="post" class="inline-block" onsubmit="return confirm('Hapus panduan ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-gray-500">Belum ada panduan produk.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
